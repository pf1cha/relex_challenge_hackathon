...
try{
 ...
 await p.reload();await p.getByText(/Project cleanup is in progress/).first().waitFor();check(await p.getByRole("button",{name:"Upload",exact:true}).isDisabled(),"Barrier upload enabled");
