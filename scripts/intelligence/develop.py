...
class ScriptedProvider:
    ...
    async def generate(self,role,system,payload,**kwargs):
        ...
        if role=='review':return {'results':[{'claim_id':c['id'],'verdict':'pass','reason_code':'supported','receipt_ids':c['receipt_ids'],'repair_request':None} for c in payload['candidate']['claims']]}
        sources=[r['result']['record_page'] for r in payload['tool_results'] if r['tool']=='read_record']
        if not sources:return {'claims':[],'cannot_establish':'no_evidence'}
...
async def main():
    ...
    limits=RuntimeLimits(answer_search_rounds=3,repair_search_rounds=1,reviewer_passes=2,reviewer_search_rounds=3,tool_calls_per_phase=40,pages_per_phase=30,source_tokens_per_phase=100000,request_deadline_seconds=180)
    ...
    try:
        ...
        candidate=await service.answer(ctx,AnswerInput(question=NormalizedText(text=case['question'],person_ids=[],ambiguous=False),history=[],request_id='request',conversation_id='conversation',deadline=datetime.now(timezone.utc)+timedelta(seconds=180)))
