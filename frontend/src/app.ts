...
function select(label:string, values:[string,string][], value=""){const n=el("select");n.setAttribute("aria-label",label);for(const [v,t]of values){const o=el("option",t);o.value=v;n.append(o);}n.value=value;return n;}
...
function showError(error:unknown,target:HTMLElement=root.querySelector("#notice")||root){
 ...
 if(error instanceof ApiError&&error.status===401){me=null;clearSession();render();}
}
...
function render(){
 ...
 root.append(top);const notice=el("div");notice.id="notice";notice.setAttribute("role","alert");root.append(notice);
 if(!me){login();return;}
 ...
 nav.append(button(name[0].toUpperCase()+name.slice(1),()=>{view=name;renderView();},view===name?"selected":""));
 root.append(nav);const main=el("main");main.id="content";root.append(main);renderView();
}
...
async function loadProjects(){projects=await list<Project>("/api/projects");const source=location.pathname.match(/^\/projects\/([^/]+)\/sources\/([^/]+)$/);
 project=(source?projects.find(p=>p.id===decodeURIComponent(source[1])):projects[0])||null;render();
...
async function renderView(){const main=root.querySelector<HTMLElement>("#content");if(!main)return;const mark=epoch;main.replaceChildren(el("p","Loading…"));
...
async function refreshJobs(main:HTMLElement,mark:number){let section=main.querySelector<HTMLElement>("[data-jobs]");
 ...
 const update=async()=>{const jobs=await list<Job>(base()+"/jobs");current(mark);if(!section?.isConnected)return;section.replaceChildren(el("h2","Processing jobs"),...jobs.map(jobRow));};
...
async function recordView(id:string){const main=root.querySelector<HTMLElement>("#content")!;main.replaceChildren(el("h2","Source record"));
 ...
 spans.append(el("h3",page.record.title));for(const s of page.spans)spans.append(el("p",s.text));cursor=page.next_cursor;more.hidden=!cursor;spans.append(el("small",page.complete?"End of record":"More source content available"));}await load();}
async function sourceView(id:string,version:number,span:string,cursor?:string){const main=root.querySelector<HTMLElement>("#content")!;main.replaceChildren(el("p","Loading source…"));
 try{const page=await api<SourcePage>(base()+"/sources/"+encodeURIComponent(id)+"?"+new URLSearchParams({version:String(version),span,...(cursor?{cursor}:{})}));main.replaceChildren(el("h2",page.record.title),el("p",page.record.record_type+" · "+(page.record.source_time.value||"Date unknown")));
 for(const item of page.spans){const n=el(page.highlighted_span_ids.includes(item.span_id)?"mark":"p",item.text);n.id="span-"+item.span_id;main.append(n);}
 ...
 }catch(e){showError(e,main);main.append(button("Return to chat and regenerate",()=>{view="chat";renderView();}));}}
...
async function citation(answerId:string,receiptId:string,anchor:HTMLElement){document.querySelector("#receipt")?.remove();const panel=el("aside");panel.id="receipt";panel.setAttribute("role","dialog");panel.setAttribute("aria-label","Source receipt");panel.append(el("p","Checking current source…"));root.append(panel);
 ...
 panel.replaceChildren(button("Close",()=>panel.remove()),el("h3",receipt.source_title),el("blockquote",receipt.quote));
 const link=el("a","Open cited source in new tab");link.href=receipt.source_url;link.target="_blank";link.rel="noopener";panel.append(link);
...
async function chat(main:HTMLElement,mark:number){main.replaceChildren(el("h2","Reviewed project chat"));
 ...
 else messages.append(el("p",m.text||m.unavailable_reason||m.state));}}
 ...
 }catch(e){processing.remove();showError(e);if(e instanceof ApiError&&["evidence_changed","answer_unavailable","idempotency_conflict"].includes(e.code))pending=null;send.textContent=pending?"Retry question":"Ask";}finally{send.disabled=false;}};}
async function overview(main:HTMLElement,mark:number){const data=await api<Models["Overview"]>(base()+"/overview");current(mark);main.replaceChildren(el("h2","Project overview"),el("p","State: "+data.state));
 ...
 const detail=el("blockquote",receipt.quote),link=el("a",receipt.source_title);link.href=receipt.source_url;link.target="_blank";link.rel="noopener";detail.append(link);row.append(detail);});row.append(b);}main.append(row);}}
...
async function administration(main:HTMLElement,mark:number){main.replaceChildren(el("h2","Project administration"),el("p","Membership grants login access. Personnel associations and person erasure are separate."));
 ...
 const memberForm=el("form"),user=input("Existing user ID"),role=select("Role",[["member","Member"],["admin","Admin"]]),save=el("button","Set membership");save.type="submit";memberForm.append(user,role,save);
 memberForm.onsubmit=async e=>{e.preventDefault();try{await api(base()+"/members","POST",{user_id:user.value,role:role.value});renderView();}catch(e){showError(e);}};main.append(memberForm,el("h3","People"));
 await pager<Models["Person"]>(main,base()+"/people",p=>{const row=el("article",p.display_name+" · "+p.kind+" · "+p.id+" · "+p.state);
 row.append(button("Erase personal information",async()=>{if(!confirm("Erase personal information for "+p.display_name+" ("+p.id+") in "+project?.name+"? Project decisions will remain with [deleted user] attribution."))return;row.append(jobRow(await api<Job>(base()+"/people/"+encodeURIComponent(p.id)+"/erase","POST")));}));
 return row;});
