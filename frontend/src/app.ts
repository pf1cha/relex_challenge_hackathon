...
type OriginalDocument=Models["OriginalDocument"]; type OriginalDocumentContent=Models["OriginalDocumentContent"];
type IdentityMapping=Models["IdentityMapping"];
type StaleReview=Models["StaleReview"];
...
type DirectoryIdentity={id:string;display_name:string;kind:"client"|"employee";contacts:Models["Contact"][];state:"active"|"erasing";action?:HTMLElement};
function identityCard(identity:DirectoryIdentity,action?:HTMLElement){const row=el("article","","identity-row"),heading=el("div","","identity-heading"),copy=el("div");copy.append(el("strong",identity.id),el("span",identity.display_name));const controls=el("div","","identity-actions");controls.append(el("span",identity.kind,"role-badge"));if(action)controls.append(action);heading.append(copy,controls);row.append(heading);const contacts=el("dl","","identity-contacts");if(identity.contacts.length)for(const contact of identity.contacts){contacts.append(el("dt",contact.kind.replaceAll("_"," ")),el("dd",contact.value));}else contacts.append(el("dt","Contact"),el("dd","No contact values associated"));row.append(contacts);return row;}
function identityGroups(container:HTMLElement,items:DirectoryIdentity[],open=false){container.replaceChildren();if(!items.length){container.append(emptyState("No identities found","Try a different pseudonym, name, or contact value."));return;}const groups=new Map<DirectoryIdentity["kind"],DirectoryIdentity[]>();for(const item of items){const group=groups.get(item.kind)||[];group.push(item);groups.set(item.kind,group);}for(const [kind,groupItems] of [...groups].sort(([a],[b])=>a.localeCompare(b))){const {group,body}=disclosure(kind==="client"?"Clients":"Employees",groupItems.length,"record-type-group identity-kind-group",open);for(const item of groupItems)body.append(identityCard(item,item.action));container.append(group);}}
function staleReviewCard(review:StaleReview,onDecision:()=>void){
 const row=el("article","","identity-row stale-review-row"),heading=el("div","","identity-heading"),copy=el("div");
 copy.append(el("strong",review.kind.replaceAll("_"," ")),el("span",review.scope));
 const controls=el("div","","identity-actions"),badge=el("span",review.state,"role-badge");controls.append(badge);
 if(review.state==="pending"){
  controls.append(button("Approve stale marking",async()=>{if(!confirm("Approve this stale marking? The confirmed relationship will become available to future answers."))return;await api<StaleReview>(base()+"/stale-reviews/"+encodeURIComponent(review.id)+"/approve","POST");onDecision();},"button-secondary"));
  controls.append(button("Reject stale marking",async()=>{if(!confirm("Reject this stale marking?"))return;await api<StaleReview>(base()+"/stale-reviews/"+encodeURIComponent(review.id)+"/reject","POST");onDecision();},"button-danger"));
 }
 heading.append(copy,controls);row.append(heading,el("p",review.proposed_change,"stale-review-change"),el("p",review.reason,"stale-review-reason"));
 const evidence=el("div","","stale-review-evidence");
 const evidenceGroup=(label:string,items:Models["Receipt"][])=>{const section=el("section");section.append(el("h4",label));for(const receipt of items){const card=el("div","","stale-evidence-card");card.append(el("strong",receipt.source_title),el("small",receipt.record_type+" · "+(receipt.source_time.value||"Date unknown")),el("blockquote",receipt.quote));const link=el("a","Open source");link.href=sourceLink(receipt);link.target="_blank";link.rel="noopener";card.append(link);section.append(card);}return section;};
 evidence.append(evidenceGroup("Document proposed as stale",review.stale_evidence),evidenceGroup("Later evidence supporting the marking",review.marking_evidence));row.append(evidence);return row;
}
...
async function documents(main:HTMLElement,mark:number){main.replaceChildren(pageHeader("Documents",pageDescriptions.documents,"Library"));
 ...
 for(const [recordType,items] of [...groups].sort(([a],[b])=>a.localeCompare(b))){const {group,body}=disclosure(recordType,items.length,"record-type-group");for(const doc of items){const row=el("article","","document-card"),mainRow=el("div","","document-card-main"),copy=el("div","","document-card-copy"),meta=el("div","","card-meta"),actions=el("div","","document-actions"),target=el("section","","document-records");meta.append(el("span",doc.ai_status,"meta-chip"),el("span",doc.processing_state,"meta-chip"));copy.append(el("h3",doc.title),el("p",doc.record_count+" source records","muted"),meta);target.hidden=true;let recordsLoaded=false,recordsLoading=false;
 const loadRecords=async()=>{if(recordsLoaded||recordsLoading)return;recordsLoading=true;openRecords.disabled=true;target.replaceChildren(el("p","Loading records...","muted"));try{target.replaceChildren();await pager<Models["RecordSummary"]>(target,base()+"/documents/"+encodeURIComponent(doc.id)+"/records",r=>{const entry=el("div");entry.append(button(r.title,()=>recordView(r.record_id)));return entry;});recordsLoaded=true;}catch(error){target.replaceChildren();showError(error,target);target.append(button("Retry",loadRecords,"button-secondary"));}finally{recordsLoading=false;openRecords.disabled=false;}};
 const openRecords=button("Open records",async()=>{const opening=target.hidden;target.hidden=!opening;openRecords.textContent=opening?"Hide records":"Open records";openRecords.setAttribute("aria-expanded",String(opening));if(opening)await loadRecords();});openRecords.setAttribute("aria-expanded","false");actions.append(openRecords);
...
async function administration(main:HTMLElement,mark:number){main.classList.add("administration-page");main.replaceChildren(pageHeader("Project administration",pageDescriptions.administration,"Admin tools"),el("p","Membership controls access to this workspace. Personnel associations and erasure remain separate.","section-note"));
 ...
 const originalsPanel=el("section","","admin-section original-documents-panel"),mappingsPanel=el("section","","admin-section identity-mappings-panel"),accessPanel=el("section"),peoplePanel=el("section","","admin-section people-directory-panel"),stalePanel=el("section","","admin-section stale-reviews-panel"),typesPanel=el("section");main.append(originalsPanel,mappingsPanel,accessPanel,peoplePanel,stalePanel,typesPanel);
 originalsPanel.append(el("h3","Original documents"),el("p","Sanitized document content. Only project administrators can open this view.","muted"));
 const originals=await list<OriginalDocument>(base()+"/original-documents"),originalLibrary=el("div","","original-document-library");
 const originalGroups=new Map<string,OriginalDocument[]>();for(const document of originals){const items=originalGroups.get(document.record_type)||[];items.push(document);originalGroups.set(document.record_type,items);}
 if(!originals.length)originalLibrary.append(emptyState("No original documents","Upload a document to add restricted source material."));
 for(const [recordType,items] of [...originalGroups].sort(([a],[b])=>a.localeCompare(b))){const {group,body}=disclosure(recordType,items.length,"record-type-group");for(const document of items){const row=el("article","","document-card original-document-card"),mainRow=el("div","","document-card-main"),copy=el("div","","document-card-copy"),actions=el("div","","document-actions"),content=el("pre","","original-document-content");content.hidden=true;copy.append(el("h3",document.title),el("p",document.processed_filename,"muted"));let loaded=false;const open=button("View document",async()=>{if(!content.hidden){content.hidden=true;open.textContent="View document";open.setAttribute("aria-expanded","false");return;}open.disabled=true;try{if(!loaded){const detail=await api<OriginalDocumentContent>(base()+"/original-documents/"+encodeURIComponent(document.id));current(mark);content.textContent=detail.processed_content;loaded=true;}content.hidden=false;open.textContent="Hide document";open.setAttribute("aria-expanded","true");}finally{open.disabled=false;}},"button-secondary");open.setAttribute("aria-expanded","false");actions.append(open);mainRow.append(copy,actions);row.append(mainRow,content);body.append(row);}originalLibrary.append(group);}originalsPanel.append(originalLibrary);
 mappingsPanel.append(el("h3","Identifier mappings"),el("p","Look up the real identity and contact values represented by a pseudonym in processed evidence.","muted"));
 const mappingForm=el("form","","identity-search-form"),mappingQuery=input("Search pseudonym or identity"),mappingSearch=el("button","Search mappings"),mappingResults=el("section","","identity-results");mappingSearch.type="submit";mappingQuery.maxLength=500;mappingForm.append(field("Pseudonym, name, or contact",mappingQuery),mappingSearch);mappingsPanel.append(mappingForm,mappingResults);
 const loadMappings=async()=>{mappingResults.replaceChildren(el("p","Searching...","muted"));const query=mappingQuery.value.trim(),items=await list<IdentityMapping>(base()+"/identity-mappings"+(query?"?q="+encodeURIComponent(query):""));current(mark);identityGroups(mappingResults,items.map(mapping=>({id:mapping.pseudonym,display_name:mapping.display_name,kind:mapping.kind,contacts:mapping.contacts,state:mapping.state})),!!query);};mappingForm.onsubmit=async event=>{event.preventDefault();mappingSearch.disabled=true;try{await loadMappings();}catch(error){showError(error,mappingResults);}finally{mappingSearch.disabled=false;}};await loadMappings();
 ...
 memberForm.onsubmit=async e=>{e.preventDefault();try{const identity=user.value.trim(),target=adminProjects.find(item=>item.id===targetProject.value);if(!target)throw new Error("Select a project you administer.");if(!confirm("Grant "+role.value+" access to "+identity+" in "+target.name+"?"))return;save.disabled=true;await api(projectBase(target.id)+"/members","POST",{...(identity.includes("@")?{email:identity}:{user_id:identity}),role:role.value});current(mark);user.value="";if(target.id===project?.id)await renderView();else memberResult.replaceChildren(el("p","Access was granted in "+target.name+".","section-note"));}catch(e){showError(e,memberResult);}finally{save.disabled=false;}};accessPanel.append(memberForm);peoplePanel.append(el("h3","People directory"),el("p","Associate clients and employees, or start a privacy erasure workflow.","muted"));
 const people=await list<Models["Person"]>(base()+"/people"),peopleForm=el("form","","identity-search-form"),peopleQuery=input("Search people directory"),peopleSearch=el("button","Search people"),peopleResults=el("section","","identity-results");peopleSearch.type="submit";peopleQuery.maxLength=500;peopleForm.append(field("Name, pseudonym, or contact",peopleQuery),peopleSearch);peoplePanel.append(peopleForm,peopleResults);
 const drawPeople=()=>{const query=peopleQuery.value.trim().toLocaleLowerCase(),matches=people.filter(person=>!query||[person.id,person.display_name,person.kind,...person.contacts.map(contact=>contact.value)].some(value=>value.toLocaleLowerCase().includes(query)));const rows=matches.map(person=>{const erase=button("Erase personal information",async()=>{if(!confirm("Erase personal information for "+person.display_name+" ("+person.id+") in "+project?.name+"? Project decisions will remain with [deleted user] attribution."))return;const job=await api<Job>(base()+"/people/"+encodeURIComponent(person.id)+"/erase","POST");await renderView();return job;},"button-danger");erase.disabled=person.state==="erasing";return {...person,action:erase};});identityGroups(peopleResults,rows,!!query);};peopleForm.onsubmit=event=>{event.preventDefault();drawPeople();};drawPeople();
 ...
 form.append(id,name,kind,contactKind,contact,add);form.onsubmit=async e=>{e.preventDefault();try{await api(base()+"/people","POST",{...(id.value?{person_id:id.value}:{}),display_name:name.value,kind:kind.value,contacts:contact.value?[{kind:contactKind.value,value:contact.value}]:[]});renderView();}catch(e){showError(e);}};peoplePanel.append(form);
 stalePanel.append(el("h3","Stale information review"),el("p","AI can propose that an older decision is stale, but it is never applied until an administrator confirms the reasoning and evidence.","muted"));
 const staleResults=el("section","","identity-results");stalePanel.append(staleResults);
 const loadStaleReviews=async()=>{staleResults.replaceChildren(el("p","Loading stale-marking proposals…","muted"));const reviews=await list<StaleReview>(base()+"/stale-reviews");current(mark);staleResults.replaceChildren(...(reviews.length?reviews.map(review=>staleReviewCard(review,()=>loadStaleReviews())):[emptyState("No stale-marking proposals","Approved and rejected proposals will remain here for audit history.")]));};
 await loadStaleReviews();await refreshJobs(main,mark);
}
