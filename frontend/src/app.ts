...
async function administration(main:HTMLElement,mark:number){main.classList.add("administration-page");main.replaceChildren(pageHeader("Project administration",pageDescriptions.administration,"Admin tools"),el("p","Membership controls access to this workspace. Personnel associations and erasure remain separate.","section-note"));
 ...
 originalsPanel.append(el("h3","Original documents"),el("p","Restricted source uploads. Only project administrators can open this content.","muted"));
 ...
 for(const [recordType,items] of [...originalGroups].sort(([a],[b])=>a.localeCompare(b))){const {group,body}=disclosure(recordType,items.length,"record-type-group");for(const document of items){const row=el("article","","document-card original-document-card"),mainRow=el("div","","document-card-main"),copy=el("div","","document-card-copy"),actions=el("div","","document-actions"),content=el("pre","","original-document-content");content.hidden=true;copy.append(el("h3",document.title),el("p",document.raw_filename,"muted"));let loaded=false;const open=button("View original",async()=>{if(!loaded){open.disabled=true;try{const detail=await api<OriginalDocumentContent>(base()+"/original-documents/"+encodeURIComponent(document.id));current(mark);content.textContent=detail.raw_content;loaded=true;}finally{open.disabled=false;}}content.hidden=!content.hidden;open.textContent=content.hidden?"View original":"Hide original";},"button-secondary");actions.append(open);mainRow.append(copy,actions);row.append(mainRow,content);body.append(row);}originalLibrary.append(group);}originalsPanel.append(originalLibrary);
