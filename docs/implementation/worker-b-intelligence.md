...
Implement answer-agent tools with progressive disclosure: `search_memory` returns only level 1 descriptions and record IDs; `read_memory(record_id)` exposes level 2 for a selected record; `search_sources` searches level 3 and returns matched source candidates; `read_record` reads canonical spans from a selected record; `get_decision_history(topic_id, scope, as_of)` and `expand_context(record_id, span_id)` provide targeted follow-up. The application may perform the initial L1 discovery, but it must not automatically append L2 summaries or L3 spans to the model prompt. Agent arguments cannot supply user/project permissions or arbitrary SQL. Tool calls carry server-bound context; document instructions are untrusted data. The reviewer is tool-free and evaluates the resulting packet.
...
Use at most three initial answer search/retrieval rounds and two review-driven answer-agent repair cycles as configurable defaults, with separate phase budgets defined below. Initial L1 discovery consumes one round; explicit retrieval state and shallow-finalization guidance encourage the remaining rounds to inspect L2 and search L3 sources, including later changes. Reading a selected record may paginate within its page/token budget. Retrieve relevant later changes as well as the initial match. Store evidence-linked history by topic/scope with source time, effective time and learned time distinct. Never infer authority solely from upload time, document type, repetition or a model confidence score.

Account budgets separately: initial answer has up to three search rounds and each of two targeted repairs has one additional search round. There are three tool-free reviewer passes: after the draft and after each repair. Use explicit finite tool/page/token caps per phase and a request-wide deadline; log every counter. Each repair retains prior evidence and receives only structured reviewer feedback. Exhaustion withholds dependent claims and never bypasses review. Fix fixture dates to year 2026 with an explicit timezone where present; as-of uses supported effective time, not what the system knew at ingestion time.
...
Reviewer uses a separate, tool-free context with no draft-agent reasoning. It receives the exact answer-agent retrieval packet, judges whether that retrieval is sufficient, and checks the draft against its canonical sources/history/counterevidence. Return `{retrieval:{verdict,missing_context,suggested_queries,suggested_record_ids}, results:[{claim_id,verdict,reason_code,receipt_ids,repair_request}]}`. The reviewer diagnoses only; the answer agent performs every additional retrieval and rewrite. Reasons include missing support, attribution mismatch, proposal as agreement, missing condition, stale as current, citation mismatch. Verify every factual clause.

Application checks span existence, quote equality, source versions and dependencies independently of the reviewer. Up to two targeted answer-agent repairs may retrieve/rewrite failed claims, each followed by a fresh review. Omit unresolved failed claims or return inability. Provider/reviewer failure must never become a verified answer. C receives only reviewed candidates; A's final gate is still required before rendering/persistence. Stream progress only, never unreviewed factual claims.
